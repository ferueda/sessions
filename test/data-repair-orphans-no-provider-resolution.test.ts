...
describe("fresh data repair-orphans composition", () => {
  test("does not resolve malformed provider configuration or create state", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "sessions-repair-orphans-lazy-source-"));
    ...
    const result = spawnSync(
      process.execPath,
      [binary, "data", "repair-orphans", "--format", "json"],
      {
        cwd: root,
        encoding: "utf8",
        env: {
