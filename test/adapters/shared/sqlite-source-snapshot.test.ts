...
describe("SQLite source snapshot", () => {
  test("materializes committed state from a main-only provider database", async () => {
  ...
  test("copies a zero-byte main file and leaves schema classification to the reader", async () => {
    const providerDirectory = await temporaryDirectory("sessions-sqlite-source-provider-");
    const databasePath = path.join(providerDirectory, "empty.sqlite");
    ...
    await expect(
      materializeSqliteSourceSnapshot({
        ...
        materialize(database) {
          const row = database.prepare("SELECT count(*) AS count FROM sqlite_schema").get() as
  ...
  test("reads committed WAL frames when the provider has no SHM", async () => {
    ...
    const providerDirectory = await temporaryDirectory("sessions-sqlite-source-no-shm-");
  ...
  test("retries provider-file replacement and reads only the replacement", async () => {
    const providerDirectory = await temporaryDirectory("sessions-sqlite-source-provider-");
    const databasePath = await closedGenerationFixture(1, providerDirectory, "state.sqlite");
    const replacementPath = await closedGenerationFixture(
      2,
      providerDirectory,
      "replacement.sqlite",
    );
    const replacedPath = path.join(providerDirectory, "replaced.sqlite");
    const workspace = await testWorkspace();
    let materializations = 0;
  ...
  test("retries WAL disappearance and accepts the next complete main-only generation", async () => {
    const origin = await activeWalFixture();
    const providerDirectory = await temporaryDirectory("sessions-sqlite-source-provider-");
    const databasePath = path.join(providerDirectory, "state.sqlite");
    await copyFile(origin.databasePath, databasePath);
    await copyFile(`${origin.databasePath}-wal`, `${databasePath}-wal`);
    const workspace = await testWorkspace();
    let materializations = 0;
  ...
  test("fails closed after exactly three changing attempts", async () => {
    ...
    expect(error).toMatchObject({
      kind: "source-changed",
      message: "SQLite source changed while it was read",
    });
...
async function closedGenerationFixture(
  ...
  const providerDirectory =
    directory ?? (await temporaryDirectory("sessions-sqlite-source-provider-"));
  const databasePath = path.join(providerDirectory, filename);
  const database = new DatabaseSync(databasePath);
  try {
    createGenerationSchema(database);
    writeGeneration(database, generation);
  } finally {
    database.close();
  }
  return databasePath;
...
async function activeWalFixture(): Promise<ActiveWalFixture> {
  const directory = await temporaryDirectory("sessions-sqlite-source-provider-");
...
function createGenerationSchema(database: DatabaseSync): void {
  database.exec(`
    CREATE TABLE left_state (
      singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
      generation INTEGER NOT NULL
    ) STRICT;
    CREATE TABLE right_state (
      singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
      generation INTEGER NOT NULL
    ) STRICT;
...
async function testWorkspace(): Promise<TestWorkspace> {
  const root = await temporaryDirectory("sessions-sqlite-source-scratch-");
